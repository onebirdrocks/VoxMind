...
struct StoryDetailView: View {
    ...
    private func generateTitleAndSummaryForStory() async {
        ...
        if apiKey.isEmpty {
            print("❌ DeepSeek API Key not configured. Please set it in Settings.")
            await MainActor.run {
        ...
        // 显示生成中状态
        await MainActor.run {
            story.originalSummary = "🤖 正在使用 AI 生成标题和摘要..."
            story.chineseSummary = "🤖 正在使用 AI 生成标题和摘要..."
        }
    ...
    // 统一的标题和摘要生成方法
    private func generateTitleAndSummary(for text: String) async throws -> TitleAndSummaryResponse {
        let apiKey = UserDefaults.standard.string(forKey: "DeepSeekAPIKey") ?? ""
